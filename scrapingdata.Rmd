...
The dataset is currently a matrix, which is not super convenient, so let's cast it to tibble and then clean it up
...
## Revert Age from factor and label Method column
...
```
Clearly there are inter-annual differences in how the date was formatted.
...
```{r}
write.csv(data,"data/policekillings.csv")
```
